...
M13: backend libghostty-vt extraction [current correctness milestone]
  ...
  nmux --state preserves cached title, OSC 7 working directory, terminal modes including mouse tracking mode/format, row runs, style tables, terminal color state, OSC 133 row/run semantic metadata, row dirty flags, row state hashes, Kitty placeholder row metadata, and distinct last-seen scrollback range metadata for patchable reconnects, validates cached terminal enum values on load, and scopes caches to the daemon socket identity so recreated socket paths force a fresh snapshot
