...
M12: live workspace usability [done]
  ...
  nmux --live --redraw --speculative-echo enables an experimental client-local overlay that predicts one outstanding printable append from the confirmed cursor position, repaints immediately, and reconciles against the next daemon-owned SurfaceUpdate without mutating ClientAttachState, protocol frames, scrollback, or daemon terminal state
