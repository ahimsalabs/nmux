...
M13: backend libghostty-vt extraction [current correctness milestone]
  ...
  decoded and cached clients reject unknown SurfaceKind, PatchKind, cursor-shape, mouse-mode, and mouse-format values, and reject row payloads on CursorOnly, ModeOnly, and ColorOnly no-row patches so version advancement cannot silently discard unexpected row changes
