...
M13: backend libghostty-vt extraction [current correctness milestone]
  ...
  optional libghostty-vt coverage includes cursor visibility/shape/blink state, alternate-screen entry/restoration with alternate scrollback omission, combining marks, emoji ZWJ clusters, basic SGR style flags, underline color, palette-indexed SGR color resolution, terminal color state, render-state default colors/palette, palette overrides, explicit cursor color, title metadata, working-directory metadata plumbing, OSC 133 row semantic prompt state, OSC 133 per-run semantic content, row-level dirty state, Kitty graphics placeholder metadata, hyperlink presence, bracketed paste, paste safety validation, local PasteInput forwarding, mouse tracking and mode-gated MouseInput forwarding, focus reporting and mode-gated FocusInput forwarding, common named-key forwarding, application keypad tracking and mode-aware keypad Enter/digit forwarding, application cursor tracking and mode-aware arrow-key forwarding, engine-backed named-key encoding with modifier preservation, explicit encoder output, origin, and wraparound modes, and mode-aware key encoding
  ...
  PaneSurfaceSnapshot and PaneSurfacePatch carry terminal title and working-directory metadata; backend-populated OSC 7 working-directory extraction remains unproven in the current libghostty-vt path
  ...
  nmux --state preserves cached row runs, style tables, and terminal color state for patchable reconnects
