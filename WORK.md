...
M12: live workspace usability [done]
  ...
  nmux --live --redraw --key TEXT --speculative-echo enables an experimental client-local overlay that predicts one outstanding printable append from the confirmed cursor position, repaints immediately, and reconciles against the next daemon-owned SurfaceUpdate without mutating ClientAttachState, protocol frames, scrollback, or daemon terminal state; raw stdin-byte mode remains outside the first SLE scope
...
M14: post-M13 promotion and product split [decision accepted in ADR 0023]
  ...
  ADR 0027 proposes speculative local echo as a client-local overlay only: predictions must not mutate confirmed ClientAttachState rows, surface versions, row hashes, scrollback state, or daemon-owned terminal state, and the first code scaffold now keeps printable ASCII append predictions separate from confirmed surfaces while reconciling against authoritative SurfaceUpdate frames
