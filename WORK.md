...
follows; clients reject a missing, extra, or wrong-kind surface frame instead of
guessing a recovery path. Future frontend handshakes may grow beyond that:
...
M10: live local interactive attach [done]
  ...
  interactive --stdin-bytes temporarily uses raw stdin with local echo defaulting off and `--local-echo tty` available
...
M12: live workspace usability [done]
  ...
  nmuxd live mode flags fail fast on ambiguous server modes and flag-specific invalid-number or zero live counts
  ...
  nmux and nmuxd informational flags exit before mode validation or socket/state/PTY work
  ...
  nmuxd --cwd validates an existing local pane working directory, and repeatable nmuxd --env KEY=VALUE configures local pane command launch context before daemon-owned NMUX_* identity variables are injected
  ...
  nmux --start starts a private managed nmuxd --one-shot --ready-json, or nmuxd --live-forever --ready-json when paired with --live, on a short temporary socket path, waits for daemon readiness internally with configurable --startup-timeout-ms, validates managed --cwd as an existing directory, forwards managed --cwd/--env launch context when supplied, reports managed --start --json startup failures as clean client JSON errors using the daemon readiness message, attaches, and cleans up the managed daemon when the client exits; nmux --shell is the one-flag interactive shorthand over that managed live path
  ...
  nmux --live --redraw --key TEXT --speculative-echo enables an experimental client-local overlay that predicts one outstanding printable append from the confirmed cursor position, repaints the predicted glyph underlined immediately, backs off after repeated misses with a short skipped-key recovery path, and reconciles against the next daemon-owned SurfaceUpdate without mutating ClientAttachState, protocol frames, scrollback, or daemon terminal state; raw stdin-byte mode remains outside the first SLE scope
  live attach wakes on local PTY output through a ProcessOutput notification FD backed by LocalPtyHost's self-pipe and poll(2), drains available client frames before forwarding input, preserves the background split-write coalescing window, and uses a shorter post-input poll-backed settle window for lower interactive latency without changing one-shot attach polling
  interactive --stdin-bytes now uses raw stdin so signal-generating control keys and flow-control keys reach the client byte stream, the default interim engine forwards common modified named keys and SGR mouse events through daemon-owned input gates, and redraw byte mode mirrors daemon mouse-tracking state onto the host terminal so --shell can receive mouse and scroll sequences
  ...
  make local-smoke runs a default-engine local daemon/client live workflow through a temporary socket and persisted state file, verifies piped stdin input produces echoed output, verifies a sequential read-only reattach sees that output, verifies nested nmux --print-context output sees the pane identity environment, verifies JSON informational flags, daemon ready-json output, and managed nmux --start --json, then reuses the same socket path for a fresh daemon and verifies stale cached surfaces do not leak across socket recreation
...
M13: backend libghostty-vt extraction [done for opt-in correctness milestone]
  ...
  pane-scoped one-shot and live client intents for unknown panes return protocol PaneNotFound errors instead of hanging or falling through to process-host behavior, decoded input/resize/scrollback-fetch intents reject missing or empty pane/actor IDs before they can target an accidental empty pane, decoded attach requests and presence updates reject missing or empty actor/user/display identity and focused/known-surface pane metadata instead of substituting local defaults, decoded workspace/surface/attach-status/scrollback state rejects missing or empty session/tab/pane IDs instead of creating empty client cache keys, attach setup rejects missing active-tab or active-pane metadata instead of guessing pane-1, and clients treat AttachStatus.pane_id, surface_version, and surface_state as authoritative for current-surface cache lookup plus post-attach input/resize/scrollback targets while rejecting missing, extra, wrong-kind, or wrong-pane following surface frames
  public scrollback ranges consistently use 1-based line numbers from ScrollbackFetch through ScrollbackChunk and decoded client summaries, rendered CLI headers report the actual returned row range, and decoded clients reject zero start/count, zero row line values, noncontiguous row line values, and row lines beyond total_lines instead of normalizing malformed chunks
  ...
  nmux --state preserves distinct last-seen scrollback range/version metadata entries scoped to the daemon socket identity, current-surface reconnects still fetch daemon-owned scrollback, matching later non-empty fetches use cached versions as preconditions, empty out-of-range chunks are not persisted as zero-count preconditions, and stale fetches retry once with no version precondition
...
M14: post-M13 promotion and product split [decision accepted in ADR 0023]
  ...
  make promotion-cold-deps-sample clears target/promotion-cold-deps and times make check-all with isolated repo-owned CARGO_HOME and CARGO_TARGET_DIR, GHOSTTY_SOURCE_DIR unset, and GIT_CONFIG_GLOBAL=/dev/null for dependency/source-fetch evidence that still does not claim full cold machine state
  make promotion-local-sample runs source-fetch provenance, the timed validation sample, local workflow smoke, the cache-present source-fetch offline probe, and package archive runtime smoke for one local evidence pass
  make promotion-evidence-bundle runs the local sample and gathers its log, toolchain output, bundle start/completion timestamps plus elapsed duration, extracted make check-all timing, local_smoke result, persisted-reattach, print-context, ready-json, managed-start, and socket-recreation smoke subresults, source-fetch report, offline probe report, package provenance, cargo tree, package archive, bundle-relative archive checksum, observed cache-state report, VCS status report, open-work snapshot, and a bundle artifact manifest under target/promotion-evidence
  make promotion-evidence-verify checks the bundle summary, bundle timing fields, make check-all timing fields, local_smoke result, persisted-reattach, print-context, ready-json, managed-start, and socket-recreation smoke subresults, artifact files, bundle-relative summary artifact names, cache-state artifact, VCS status artifact, exact open-work snapshot lines, relocation-safe bundle artifact manifest hashes, source/provenance records, cache-present offline probe result, bundled package provenance through the no-rebuild manifest verifier, package archive bytes, archive hash, archive verifier output, package provenance verifier output from the bundle-generation run log, and packaged runtime smoke result so bundle evidence is self-checking before transcription or upload
  ...
  make source-fetch-offline-probe records a reusable cache-present offline probe artifact showing whether nmux-core --features libghostty-vt can compile with CARGO_NET_OFFLINE=true and a separate target directory, proving only that the current cache state can satisfy that opt-in build path
  ...
  make packaging-layout-sample stages opt-in release binaries, wrapper scripts, and libghostty-vt runtime-library artifacts in a local package layout, then runs the no-rebuild layout verifier against that layout
  make packaging-layout-verify validates an existing staged opt-in native-VT package layout without rebuilding, including wrapper scripts, libexec binaries, package metadata, native runtime-library presence, and wrapped binary version checks
  make packaging-provenance-sample writes a manifest for the staged opt-in package layout with file hashes, package metadata, toolchain/source mode, dependency tree, native runtime-library artifacts, and dynamic dependency output
  make packaging-provenance-verify regenerates the opt-in native-VT package provenance manifest and fails if required toolchain, source-mode, locked package, staged-file hash, package metadata, runtime-library, per-binary libghostty-vt dynamic-dependency, or cargo-tree records are missing
  make packaging-provenance-manifest-verify validates an existing opt-in native-VT package provenance manifest without rebuilding; PACKAGING_PROVENANCE_MANIFEST can point it at copied, bundled, or downloaded PACKAGE_PROVENANCE.txt evidence
  make packaging-archive-sample writes a tar archive and SHA-256 file for the staged opt-in package layout, extracts it, verifies wrapped nmux and nmuxd version checks from the extracted archive, and then runs the no-rebuild archive verifier
  make packaging-archive-verify validates an already-produced opt-in native-VT package archive plus sidecar hash without rebuilding, including archive bytes, extracted layout, package metadata, provenance file hashes, native runtime-library artifacts, dynamic dependency records, and the no-rebuild layout verifier against the extracted package layout
  make packaging-archive-runtime-smoke extracts the archive into a fresh /tmp install root with library-path environment unset, then starts the wrapped opt-in libghostty-vt daemon and attaches the wrapped client to prove the relocated package layout can serve a real pane
  current local Darwin packaging evidence covers direct default release binaries, opt-in libghostty-vt binaries with the produced runtime library path, staged wrapper layout, package provenance, archive verification, and relocated archive runtime smoke; supported targets, signing/update shape, and final runtime-library policy still need a packaging ADR before native-VT binaries are shippable
  ...
  GitHub Actions now runs the flake source audit, the default-engine make check gate, and make local-smoke on pull requests and main pushes, with make promotion-evidence-bundle available only as a manual workflow_dispatch job for CI promotion evidence; the manual job uploads target/promotion-evidence as the nmux-promotion-evidence artifact, and a dependent job downloads that artifact and runs make promotion-evidence-verify against the downloaded copy
  ...
  docs/default-engine-promotion.md also records current default-engine push samples separately from manual promotion evidence, so green README/AGENTS/docs-only pushes prove the normal make check plus local-smoke gate without implying native-VT promotion readiness
  the first manual Ubuntu promotion evidence run is recorded as passing the default-engine job, promotion-evidence bundle job, downloaded-artifact verifier job, and local downloaded-bundle verifier recheck; repeat samples, runner/cache classification, native-VT flake tracking, and GitHub Actions deprecation follow-up remain open before promotion
  ...
  nix build . now uses crane to build installable default interim-engine nmux/nmuxd binaries from the filtered flake source; checks.default validates schema plus Rust test-target compilation under Nix, while live PTY workflow execution remains in the dev-shell make gates
  ADR 0027 proposes speculative local echo as a client-local overlay only: predictions must not mutate confirmed ClientAttachState rows, surface versions, row hashes, scrollback state, or daemon-owned terminal state, and the first code scaffold now keeps printable ASCII append predictions separate from confirmed surfaces while rendering the predicted glyph underlined, backing off after repeated misses, and reconciling against authoritative SurfaceUpdate frames
  renderer-equivalence tooling now has a first oracle-directory comparison interface: make renderer-equivalence-artifacts writes nmux canonical projections, and make renderer-equivalence-compare RENDERER_EQUIVALENCE_ORACLE_DIR=... compares live CLI fixture output against matching <fixture>.canonical.json files without treating nmux self-generated artifacts as trusted renderer evidence
  renderer-equivalence work should come before more default-renderer or frontend claim promotion: define the fixture corpus, trusted oracle renderer, comparison harness, tolerance policy, regression gate, and user-facing claim map before changing those claims; the current nmux-side fixture corpus now includes SGR style flags, underline variants/colors, styled trailing blank cells, explicit cursor color, palette override, palette-indexed style resolution, cursor-editing output, combining-mark cell-width projection, explicit post-output resize/reflow, and a capability-gated Kitty placeholder metadata fixture, but still lacks the trusted oracle comparison needed for claim promotion
  ...
  README, docs/running.md, and AGENTS now keep first-reader check and quick-start guidance short, point broader target inventories to docs/toolchain.md and docs/contributor-workflow.md, and separate backend default-engine blockers from the frontend Ghostty renderer hydration track
  ...
  README and docs/running.md show the local-smoke success signature, including JSON informational, ready-json, and managed-start subchecks, so first-run users can recognize a completed default workflow through noisy build output
  ...
  docs/packaging.md now records the current no-release-binary stance, local Darwin wrapper/archive/runtime-smoke package-layout evidence, and native-VT packaging questions required before default promotion
