...
M14: post-M13 promotion and product split [decision accepted in ADR 0023]
  ...
  ADR 0027 proposes speculative local echo as a client-local overlay only: predictions must not mutate confirmed ClientAttachState rows, surface versions, row hashes, scrollback state, or daemon-owned terminal state, and the first code scaffold now keeps printable ASCII append predictions separate from confirmed surfaces while rendering the predicted glyph underlined and reconciling against authoritative SurfaceUpdate frames
