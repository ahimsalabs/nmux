...
M14: post-M13 promotion and product split [decision accepted in ADR 0023]
  ...
  GitHub Actions now runs the default-engine make check gate on pull requests and main pushes, with make promotion-evidence-bundle available only as a manual workflow_dispatch job for CI promotion evidence; the manual job uploads target/promotion-evidence as the nmux-promotion-evidence artifact
