...
M13: backend libghostty-vt extraction [current correctness milestone]
  ...
  PaneSurfaceSnapshot, PaneSurfacePatch, and ScrollbackChunk carry TerminalColorState; color-only updates use PatchKind::ColorOnly, feature-gated local live coverage proves ColorOnly patches update cached client colors from palette diffs without row repaint and survive state encode/decode, and palette overrides that alter existing row style-table entries force full refreshes
