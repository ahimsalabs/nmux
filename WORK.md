...
M12: live workspace usability [done]
  ...
  make local-smoke runs a default-engine local daemon/client live workflow through a temporary socket and persisted state file, verifies piped stdin input produces echoed output, verifies a sequential read-only reattach sees that output, verifies nested nmux --print-context output sees the pane identity environment, verifies JSON informational flags and daemon ready-json output, then reuses the same socket path for a fresh daemon and verifies stale cached surfaces do not leak across socket recreation
