...
M13: backend libghostty-vt extraction [current correctness milestone]
  ...
  SurfaceRow, RowUpdate, and ScrollbackRow carry OSC 133 row semantic prompt metadata; CellRun carries OSC 133 output/input/prompt semantic content; decoded surfaces and scrollback reject unknown semantic enum values; broader semantic command IDs, ranges, lifecycle, and exit metadata remain withheld
  ...
  decoded and cached clients reject unknown PatchKind values and row payloads on CursorOnly, ModeOnly, and ColorOnly no-row patches so version advancement cannot silently discard unexpected row changes
