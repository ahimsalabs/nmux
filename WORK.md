...
M12: live workspace usability [done]
  ...
  nmux --start starts a private managed nmuxd --one-shot --ready-json, or nmuxd --live-forever --ready-json when paired with --live, on a short temporary socket path, waits for daemon readiness internally, forwards managed --cwd/--env launch context when supplied, attaches, and cleans up the managed daemon when the client exits
