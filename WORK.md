...
M14: post-M13 promotion and product split [decision accepted in ADR 0023]
  ...
  make promotion-evidence-bundle runs the local sample and gathers its log, toolchain output, extracted make check-all timing, source-fetch report, package provenance, cargo tree, and archive checksum under target/promotion-evidence
  make promotion-evidence-verify checks the bundle summary, timing fields, artifact files, source/provenance records, archive hash, package provenance verifier output, and packaged runtime smoke result so bundle evidence is self-checking before transcription or upload
