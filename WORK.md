...
M14: post-M13 promotion and product split [decision accepted in ADR 0023]
  ...
  renderer-equivalence work should come before more default-renderer or frontend claim promotion: define the fixture corpus, trusted oracle renderer, comparison harness, tolerance policy, regression gate, and user-facing claim map before changing those claims; the current nmux-side fixture corpus now includes explicit cursor color, palette override, palette-indexed style resolution, cursor-editing output, and combining-mark cell-width projection, but still lacks the trusted oracle comparison needed for claim promotion
