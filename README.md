...
VCS status report, summary/VCS git revision agreement, open-work snapshot,
package archive bytes, archive hash, and packaged runtime smoke result. For
bundles generated by GitHub Actions, it also requires concrete run, ref, SHA,
and runner fields and checks that `github_sha` matches the bundled revision.
It invokes the no-rebuild package provenance manifest verifier against the
bundled `PACKAGE_PROVENANCE.txt`, so downloaded evidence is checked directly
instead of being replaced by freshly generated local evidence.
...
records are present.
`make packaging-provenance-manifest-verify` checks an existing provenance
manifest without rebuilding; set
`PACKAGING_PROVENANCE_MANIFEST=/path/to/PACKAGE_PROVENANCE.txt` when reviewing a
copied or bundled manifest.
`make packaging-archive-sample` archives the staged layout, writes a SHA-256
file, extracts it, verifies the wrapped binaries from the archive, and then runs
