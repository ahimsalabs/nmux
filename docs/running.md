...
For explicit live input events, `--paste TEXT`, `--focus gained|lost`, `--key-name keypad-enter|keypad-0..9`, and `--mouse action:button:row:col` send structured input frames instead of raw text. Mouse actions are `press`, `release`, or `motion`; buttons are `none`, `left`, `middle`, `right`, `wheel-up`, or `wheel-down`; row and column are 1-based cells in the CLI and are converted to zero-based protocol coordinates. Focus, keypad, paste, and mouse forwarding are encoded by the daemon from daemon-owned pane modes rather than cached client mode state.
