...
active terminal engine, or if the host refuses the forwarded bytes, the daemon
returns an `Error` frame and the CLI prints the server-provided reason instead
of reporting an ambiguous closed connection. One-shot clients also check for an
input error before requesting scrollback, so unsafe paste or encoding failures
