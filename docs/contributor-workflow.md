...
temporary socket. GitHub Actions also runs `just static-link-verify` on the
default matrix and `just check-interim` on Ubuntu. The manual promotion
...
`just check-ghostty-vt` remains as a compatibility alias for `just check`.
The serial harness setting in `just check` is part of the current native-VT
gate; do not replace it with filtered tests when changing terminal engine
behavior.

`just renderer-equivalence-smoke` is narrower: it runs the current
...
Use `just renderer-equivalence-artifacts` to write canonical nmux projections,
and `just renderer-equivalence-compare
...
| Question | Command |
| --- | --- |
| Does the combined default plus opt-in gate pass here? | `nix develop . -c just promotion-sample` |
| Does the user-level default workflow still work? | `nix develop . -c just local-smoke` |
| What changes with a fresh Rust target dir? | `nix develop . -c just promotion-cold-target-sample` |
| Can isolated Cargo dependency/source fetches satisfy the opt-in path? | `nix develop . -c just promotion-cold-deps-sample` then `nix develop . -c just promotion-cold-deps-verify` |
| Do source-fetch, validation, workflow smoke, offline probe, and package runtime smoke pass together? | `nix develop . -c just promotion-local-sample` |
| Do we need a self-contained evidence artifact? | `nix develop . -c just promotion-evidence-bundle` then `nix develop . -c just promotion-evidence-verify` |
...
`just check-all` sample is useful evidence, but it does not change the default
engine by itself.

`just local-smoke` is a quick user-level workflow check for the default engine:
...
`just promotion-sample` prints `just toolchain-info` output and then times
`just check-all` with `time -p`. Outside the Nix shell, run the same target for
non-Nix setup attempts. The justfile checks for `flatc` 25.12.19, `cargo`, and
...
`just check-all` with that fresh Rust target directory. Use it for local
cold-target evidence, and record that it does not clear Cargo registry, Git
source, or Nix store caches.

`just promotion-cold-deps-sample` clears `target/promotion-cold-deps` and times
`just check-all` with isolated repo-owned `CARGO_HOME` and `CARGO_TARGET_DIR`,
...
`just promotion-cold-deps-verify` checks the existing `REPORT.txt` and
...
`just promotion-local-sample` runs `just source-fetch-provenance-sample`,
`just promotion-sample`, `just local-smoke`, `just source-fetch-offline-probe`,
and then `just packaging-archive-runtime-smoke` with clear section headers. Use
...
`just promotion-evidence-bundle` runs the local sample and gathers
...
`SUMMARY.txt` includes the extracted `time -p just check-all` values as
...
when the bundle was generated; `just promotion-evidence-verify` checks that its
...
It runs `just promotion-evidence-verify` before printing the artifact list.
Run `just promotion-evidence-verify` directly when reviewing an existing bundle
without regenerating the native build and packaging sample; it validates the
bundled package provenance through `just packaging-provenance-manifest-verify`
and bundled archive bytes through `just packaging-archive-verify`. For
...
`just PROMOTION_EVIDENCE_DIR=/path/to/artifact promotion-evidence-verify` when
...
`just source-fetch-provenance-sample` writes the active source mode and locked
...
`just source-fetch-provenance-verify` checks an existing `SOURCE_FETCH.txt`
...
`just source-fetch-offline-probe-verify` checks the existing `OFFLINE_PROBE.txt`
...
`just packaging-sample` prints the same toolchain context, builds default and
...
`just packaging-layout-sample` stages the opt-in binaries, wrapper scripts, and
...
`just packaging-layout-verify` checks an existing staged layout without
...
`just packaging-provenance-sample` writes a manifest for that staged layout with
...
`just packaging-provenance-verify` regenerates that manifest and asserts the
...
`just packaging-archive-sample` archives the staged layout, writes an archive
...
`just packaging-layout-verify` against the extracted layout before accepting
the archive.
`just packaging-archive-runtime-smoke` extracts the archive into a fresh `/tmp`
