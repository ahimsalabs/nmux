...
- `make promotion-evidence-bundle` runs the local sample and gathers its log,
  ...
  `target/promotion-evidence`. The bundled `ARCHIVE.sha256` uses the
  bundle-relative `PACKAGE_ARCHIVE.tar.gz` path.
- `make promotion-evidence-verify` checks an existing bundle for required
  ...
  artifact, VCS status artifact, relocation-safe `BUNDLE_MANIFEST.txt` hashes,
