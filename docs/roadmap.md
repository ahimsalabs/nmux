...
- Usable today: the default `interim` engine supports local one-shot attach,
  ...
  raw interactive stdin with modified named-key and SGR mouse forwarding in the
  default engine, host-terminal mouse tracking in redraw byte mode, and
  `make local-smoke`.
...
- `nmux --live --stdin-bytes` sends stdin chunks as `InputKind.RawBytes` on a background thread and temporarily puts interactive TTY stdin into raw mode; local echo defaults off and can preserve the TTY setting with `--local-echo tty`.
- Byte-streamed live sessions support Ctrl-] as a local detach key, and
  `--detach-key none` lets the pane receive that byte instead.
...
- `nmux --no-scrollback` skips the post-attach scrollback fetch for one-shot and live attach when a user or script wants only the current surface.
...
- `make check-ghostty-vt` now runs full `nmux-core` and `nmux-cli` test suites with `--features libghostty-vt` and `RUST_TEST_THREADS=1`, so the opt-in engine gate covers ordinary feature-sensitive tests as well as Ghostty-named smoke tests while keeping the current FFI-backed native-VT harness serial.
...
- The tracker includes multiple warm local Darwin arm64 `make check-all`
...
- Keep adding supported-platform and CI results to
  [the default-engine promotion evidence tracker](default-engine-promotion.md),
  including repeat manual promotion evidence bundle workflow results,
  runner/cache classification, native-VT flake notes, and GitHub Actions
  deprecation follow-up, before proposing `libghostty-vt` as the default engine
  or a regular CI requirement.
