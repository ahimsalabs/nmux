...
- Named-key forwarding now goes through the live pane terminal engine; the `libghostty-vt` key encoder is tested for application-cursor state and modified named keys, decoded client input rejects unsupported modifier bits outside `shift`, `ctrl`, `alt`, and `super`, and `nmux --key-modifiers` exposes those modifiers for live named-key input while broader physical-key/text-event forwarding remains a protocol/frontend boundary decision.
