...
- Persisted `nmux --state` files now retain cached title, OSC 7 working directory, terminal modes, row runs, style tables, terminal color state, OSC 133 row/run semantic metadata, row dirty flags, row state hashes, and Kitty placeholder row metadata alongside fallback row text; cache reuse is scoped to the daemon socket identity, while older unscoped text-only state files still load as default-style rows and force one fresh snapshot.
