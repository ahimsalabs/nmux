...
- The CLI workspace summary displays the daemon-published pane resize policy.
...
- Continue from stdin byte streaming, opt-in redraw, committed resize display, and explicit local echo choice toward a fuller terminal frontend loop: richer layout policy and signal handling.
