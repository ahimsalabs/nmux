...
Status: Started. ADR 0012 documents the terminal engine boundary, ADR 0013 documents the optional `libghostty-vt` backend, and ADR 0015 documents per-run cell semantic content. `nmux-core` exposes a terminal engine trait, the interim text behavior implements it, pane cursor state and resize updates are routed through that engine boundary, local daemon serving paths keep engine instances alive per pane across output polls and sequential clients, and `nmuxd --terminal-engine interim` exposes the current engine choice. `libghostty-vt` is now an optional Cargo feature with a compile-checked and smoke-tested engine path. The optional engine extracts cursor state including blink, surface kind, terminal title, working-directory metadata when available, terminal modes, OSC 133 row semantic prompt state, OSC 133 per-run semantic content, row dirty state, Kitty virtual placeholder rows, style-separated visible row runs, cell widths, resize/reflow output, and backend-owned scrollback into nmux state objects; snapshots and scrollback chunks carry style tables, row semantic prompt metadata, run semantic content, row dirty flags, and Kitty placeholder metadata, snapshots and patches carry title, working-directory, and mode metadata, patches avoid unsupported style-table changes, stable-geometry row replacements emit sparse row updates, mode-only updates use explicit `PatchKind::ModeOnly` payloads, persisted client state keeps cached cursor blink, title, working-directory, mode state, row semantic prompt state, run semantic content, row dirty state, Kitty placeholder state, row runs, and style tables, alternate-screen scrollback omission is pinned while the protocol shape remains withheld, render-state tests cover default foreground/background plus the active palette as backend-observable state, local paste forwarding uses `PasteInput` and pane-mode-controlled bracketed-paste delimiters, local focus forwarding uses `FocusInput` gated by daemon-owned focus-reporting mode, common named-key forwarding avoids raw PTY replay through terminal-engine encoding, keypad Enter and digit forwarding uses daemon-owned application-keypad mode, arrow-key forwarding uses daemon-owned application-cursor mode, modified named-key protocol input is preserved for engines that can encode it, mouse press/release/motion forwarding uses daemon-owned mouse-tracking mode and terminal-engine encoding, backend-populated OSC 7 working-directory extraction remains unproven, broader semantic command IDs/ranges/lifecycle/exit metadata remains withheld, richer run/region damage fields remain withheld, image placement fields remain withheld, and broader physical-key/text-event forwarding remains withheld. The default engine remains interim until the project deliberately accepts the native Zig/Ghostty build in regular development and CI.
