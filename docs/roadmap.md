...
Status: Started. ADR 0012 documents the terminal engine boundary and ADR 0013 documents the optional `libghostty-vt` backend. `nmux-core` exposes a terminal engine trait, the interim text behavior implements it, pane cursor state and resize updates are routed through that engine boundary, local daemon serving paths keep engine instances alive per pane across output polls and sequential clients, and `nmuxd --terminal-engine interim` exposes the current engine choice. `libghostty-vt` is now an optional Cargo feature with a compile-checked and smoke-tested engine path. The optional engine extracts cursor state including blink, surface kind, terminal title, terminal modes, OSC 133 row semantic prompt state, row dirty state, Kitty virtual placeholder rows, style-separated visible row runs, cell widths, resize/reflow output, and backend-owned scrollback into nmux state objects; snapshots and scrollback chunks carry style tables, row semantic prompt metadata, row dirty flags, and Kitty placeholder metadata, snapshots and patches carry title and mode metadata, patches avoid unsupported style-table changes, mode-only updates use explicit `PatchKind::ModeOnly` payloads, persisted client state keeps cached cursor blink, title, mode state, row semantic prompt state, row dirty state, Kitty placeholder state, row runs, and style tables, alternate-screen scrollback omission is pinned while the protocol shape remains withheld, render-state tests cover default foreground/background plus the active palette as backend-observable state, local paste forwarding uses `PasteInput` and pane-mode-controlled bracketed-paste delimiters, OSC 7 working-directory state remains withheld, broader semantic input/output command metadata remains withheld, richer damage fields remain withheld, image placement fields remain withheld, explicit application-keypad encoding is covered while terminal-derived keypad forwarding remains withheld, and focus reporting and focus event encoding are covered while focus forwarding remains withheld. The default engine remains interim until the project deliberately accepts the native Zig/Ghostty build in regular development and CI.
