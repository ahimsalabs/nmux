...
- OSC 8 hyperlink text and backend row/cell hyperlink presence are covered, with run-level presence carried in `CellRun.flags` while `hyperlink_id` remains unset until nmux has a real hyperlink table; feature-gated local and live CLI coverage proves those run flags survive `ReplaceRows` cache updates, persisted state encode/decode, and real `--state` reattach.
...
- Protocol `Error` frames now carry structured pane attribution and input sequence attribution for pane-scoped/input failures, so clients do not need to parse reason strings to identify the failed pane or input event.
