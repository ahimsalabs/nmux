...
Status: Started. ADR 0012 documents the terminal engine boundary. `nmux-core` exposes a terminal engine trait, the interim text behavior implements it, pane cursor state and resize updates are routed through that engine boundary, local daemon serving paths keep engine instances alive per pane across output polls and sequential clients, and `nmuxd --terminal-engine interim` exposes the current engine choice. `libghostty-vt` is now an optional Cargo feature with a compile-checked and smoke-tested engine path, but the default engine remains interim until the project deliberately accepts the native Zig/Ghostty build in regular development and CI.
