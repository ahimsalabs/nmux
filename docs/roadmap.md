...
- `libghostty-vt` safe API title metadata is tested as backend-observable state, while OSC 7 working-directory state remains an explicit omission until upstream behavior is clear.
- `libghostty-vt` safe API OSC 133 prompt-row semantics are tested as backend-observable state, while nmux still withholds semantic prompt/input/output protocol fields.
- `libghostty-vt` render-state row dirty flags are tested as backend-observable damage state, while nmux still uses row replacement patches until richer damage protocol fields exist.
...
Status: Started. ADR 0012 documents the terminal engine boundary and ADR 0013 documents the optional `libghostty-vt` backend. `nmux-core` exposes a terminal engine trait, the interim text behavior implements it, pane cursor state and resize updates are routed through that engine boundary, local daemon serving paths keep engine instances alive per pane across output polls and sequential clients, and `nmuxd --terminal-engine interim` exposes the current engine choice. `libghostty-vt` is now an optional Cargo feature with a compile-checked and smoke-tested engine path. The optional engine extracts cursor state, surface kind, terminal modes, style-separated visible row runs, cell widths, resize/reflow output, and backend-owned scrollback into nmux state objects; snapshots and scrollback chunks carry style tables, patches avoid unsupported style-table changes, mode-only updates use explicit `PatchKind::ModeOnly` payloads, persisted client state keeps cached mode state, row runs, and style tables, alternate-screen scrollback omission is pinned while the protocol shape remains withheld, render-state tests cover default foreground/background plus the active palette as backend-observable state, title metadata is covered while OSC 7 working-directory state remains withheld, OSC 133 prompt semantics are covered while semantic row metadata remains withheld, row dirty state is covered while richer damage fields remain withheld, Kitty graphics placeholder rows are covered while image placement fields remain withheld, explicit application-keypad encoding is covered while terminal-derived keypad forwarding remains withheld, focus reporting and focus event encoding are covered while focus forwarding remains withheld, and paste safety validation is covered while paste forwarding remains withheld. The default engine remains interim until the project deliberately accepts the native Zig/Ghostty build in regular development and CI.
