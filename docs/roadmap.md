...
- The tracker includes multiple warm local Darwin arm64 `make check-all`
  ...
  the Nix shell; remaining promotion evidence still needs more local platforms,
...
- [CI notes](ci.md) document the GitHub Actions default-engine gate and manual
  promotion-local-sample job; promotion still needs recorded CI runs before the
  opt-in native VT gate can become required.
...
- Keep adding supported-platform and CI results to [the default-engine promotion evidence tracker](default-engine-promotion.md), including manual promotion-local-sample workflow results, before proposing `libghostty-vt` as the default engine or a regular CI requirement.
