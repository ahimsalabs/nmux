...
- The `nmux --follow` local loop keeps one client render state across repeated reconnects, applies snapshots/patches, and renders the scoped cached surface when a current-version reconnect has no newer surface frame.
...
- `libghostty-vt` render-state colors are carried as `TerminalColorState`: backend-observable default foreground/background, active palette, palette overrides, and explicit cursor-color state. Color-only updates use `PatchKind::ColorOnly`; feature-gated local live coverage proves color-only patches stream after attach, avoid row repaint, update cached client color state, and survive persisted state encode/decode. Palette overrides that alter existing row style-table entries and other color changes coupled to row/style changes require a full surface refresh because incremental palette diffs remain withheld.
...
- OSC 8 hyperlink text and backend row/cell hyperlink presence are covered, with run-level presence carried in `CellRun.flags` while `hyperlink_id` remains unset until nmux has a real hyperlink table; feature-gated local live coverage proves those run flags survive `ReplaceRows` cache updates and persisted state encode/decode.
...
- Feature-gated local live coverage now proves `libghostty-vt` mode-only updates stream after attach as `PatchKind::ModeOnly`, avoid row repaint, update cached client mode state, and survive persisted state encode/decode.
- The `libghostty-vt` safe API is now tested for mouse tracking modes, mouse encoding formats, and SGR mouse encoding with modifiers; nmux carries both the compatibility mouse-tracking flag and detailed tracking-mode/format state, and local live clients can forward explicit mouse press/release/motion input through daemon-owned pane-bounds validation and terminal-engine encoding.
...
- Title/OSC 7-only changes use no-row surface patches, and feature-gated live CLI coverage proves non-redraw clients print metadata-only changes without reprinting unchanged row text and persist those metadata-only changes through `--state` reattach.
...
- `libghostty-vt` render-state row dirty flags and row state hashes are carried through surface snapshots, surface patches, scrollback chunks, and cached client state; `ReplaceRows` patches now carry only changed rows when pane geometry is stable, and feature-gated local live coverage proves row semantic content, dirty state, and row_state_hash metadata update cached client state through live patches. Richer run/region damage protocol fields remain withheld.
...
- Local clients now fetch daemon-owned scrollback even when a reconnect has the current visible surface, preserve distinct scoped cached scrollback range/version entries as fetch preconditions for matching ranges, and retry once with `known_scrollback_version = 0` after `StaleVersion`.
...
- ADR 0018 keeps `libghostty-vt` opt-in for default development and regular `make check` while requiring the full feature-enabled `make check-ghostty-vt` gate for related changes; a later ADR must accept native build, CI, and packaging costs before making it regular CI or the documented default.
