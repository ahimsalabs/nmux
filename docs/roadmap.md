...
- The `libghostty-vt` safe API is now tested for mouse tracking modes, mouse encoding formats, and SGR mouse encoding with modifiers; nmux carries both the compatibility mouse-tracking flag and detailed tracking-mode/format state, rejects malformed mouse button/action enum values, and local live clients can forward explicit mouse press/release/motion input through daemon-owned pane-bounds validation and terminal-engine encoding.
