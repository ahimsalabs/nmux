...
- `nmux --start --live` starts a private managed `nmuxd --live-forever
  --ready-json`, waits for readiness internally, attaches over a short temporary
  socket path, and cleans up the managed daemon when the live client exits.
