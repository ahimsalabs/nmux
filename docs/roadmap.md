...
- The `libghostty-vt` safe API is now tested for focus reporting mode and focus event encoding, and nmux carries focus-reporting mode state while focus forwarding remains a protocol/frontend boundary decision.
- The `libghostty-vt` safe API is now tested for application keypad mode tracking, and nmux carries application-keypad mode state while keypad input forwarding remains a protocol/frontend boundary decision.
- The `libghostty-vt` key encoder is now tested for explicit application-keypad encoding, while terminal-derived keypad forwarding remains a protocol/frontend boundary decision.
- The `libghostty-vt` safe API is now tested for application cursor, origin, and wraparound modes, and nmux carries those mode states in pane surface snapshots and patches.
...
- `libghostty-vt` safe API title metadata is carried through pane surface snapshots and patches, while OSC 7 working-directory state remains an explicit omission until upstream behavior is clear.
- `libghostty-vt` OSC 133 prompt-row semantics are carried through surface snapshots, surface patches, scrollback chunks, and cached client state, while nmux still withholds broader semantic input/output command metadata.
- `libghostty-vt` render-state row dirty flags are carried through surface snapshots, surface patches, scrollback chunks, and cached client state, while nmux still uses row replacement patches until richer damage protocol fields exist.
- `libghostty-vt` safe API Kitty graphics placeholder rows are tested as backend-observable state, while nmux still withholds image placement, dimensions, pixel-data, persistence, and fallback protocol fields.
...
Status: Started. ADR 0012 documents the terminal engine boundary and ADR 0013 documents the optional `libghostty-vt` backend. `nmux-core` exposes a terminal engine trait, the interim text behavior implements it, pane cursor state and resize updates are routed through that engine boundary, local daemon serving paths keep engine instances alive per pane across output polls and sequential clients, and `nmuxd --terminal-engine interim` exposes the current engine choice. `libghostty-vt` is now an optional Cargo feature with a compile-checked and smoke-tested engine path. The optional engine extracts cursor state including blink, surface kind, terminal title, terminal modes, OSC 133 row semantic prompt state, row dirty state, style-separated visible row runs, cell widths, resize/reflow output, and backend-owned scrollback into nmux state objects; snapshots and scrollback chunks carry style tables, row semantic prompt metadata, and row dirty flags, snapshots and patches carry title and mode metadata, patches avoid unsupported style-table changes, mode-only updates use explicit `PatchKind::ModeOnly` payloads, persisted client state keeps cached cursor blink, title, mode state, row semantic prompt state, row dirty state, row runs, and style tables, alternate-screen scrollback omission is pinned while the protocol shape remains withheld, render-state tests cover default foreground/background plus the active palette as backend-observable state, OSC 7 working-directory state remains withheld, broader semantic input/output command metadata remains withheld, richer damage fields remain withheld, Kitty graphics placeholder rows are covered while image placement fields remain withheld, explicit application-keypad encoding is covered while terminal-derived keypad forwarding remains withheld, focus reporting and focus event encoding are covered while focus forwarding remains withheld, and paste safety validation is covered while paste forwarding remains withheld. The default engine remains interim until the project deliberately accepts the native Zig/Ghostty build in regular development and CI.
