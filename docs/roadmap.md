...
- Explicit one-shot `--key` and `--paste` input is forwarded before scrollback fetch even when a reconnect has the current visible surface and no surface frame is sent.
- `nmux --follow` rejects one-shot `--key` and `--paste` input flags instead of accepting and silently dropping them.
