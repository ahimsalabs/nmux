...
- OSC 8 hyperlink text and backend row/cell hyperlink presence are covered, with run-level presence carried in `CellRun.flags` while `hyperlink_id` remains unset until nmux has a real hyperlink table; ADR 0022 defines the intended table shape and conservative patching rule, and feature-gated local and live CLI coverage proves current run flags survive `ReplaceRows` cache updates, persisted state encode/decode, and real `--state` reattach.
