...
When a structured live input event cannot be encoded or safely forwarded, the
daemon sends an `Error` frame on the live stream instead of terminating the
connection with an opaque socket close. The local CLI reports that frame as a
live server error.
