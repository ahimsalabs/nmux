...
entry/restoration, resize/reflow, styled backend-owned scrollback extraction,
and safe-API mode tracking for bracketed paste, mouse tracking, application
keypad mode, and origin/wraparound modes through unit, session, and live CLI
smoke coverage. It is not the default until the project deliberately accepts the
