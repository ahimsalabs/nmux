...
- M13 backend `libghostty-vt` extraction is the current terminal-state correctness milestone. The optional engine is imported and smoke-tested, but the default engine remains `interim` until a later decision accepts the native Ghostty/Zig build in the regular path.
